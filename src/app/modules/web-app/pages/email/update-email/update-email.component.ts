...
export class UpdateEmailComponent implements OnInit, OnDestroy {
  ...
  getEmailById(emailId: number) {
    ...
    this.authService.memberSendRequest('get', 'getEmailTemplateById/' + emailId, null).subscribe((respData: any) => {
  ...
  setEmailData() {
    ...
    let tokenUrl = this.commonFunctionService.genrateImageToken(this.emailDetails.id, 'emailTemplate');
    this.emailDetails['token'] = { url: tokenUrl, isLoading: true, imageLoaded: false };
    this.commonFunctionService.loadImage(this.emailDetails['token']);

    this.updateEmailForm.controls['file'].setValue(tokenUrl);
    this.imageUrl = tokenUrl;

    if (!this.emailDetails['token']) {
  ...
  updateEmailProcess() {
    ...
    if (this.updateEmailForm.valid) {
      ...
      this.authService.setLoader(true);
      this.authService.memberSendRequest('put', 'updateEmailTemplate/' + this.emailId, formData).subscribe(
        (respData: any) => {
